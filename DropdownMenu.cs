...
public class DropdownMenu : Question {
    ...
    public override void createMenu()
    {
        ...
        myMenu.transform.SetParent(myMenu.transform.parent);
        //var newMenu = transform.GetComponent<Dropdown>();
        //newMenu.Options.Clear();
        for (int i = 0; i < myOptions.Length; i++)
        {
            //creating each option on the menu
            myMenu.GetComponent<Dropdown>().options.Add(new Dropdown.OptionData() { text = myOptions[i] });
        }
        myMenu.transform.localPosition = new Vector2(-40,80);
